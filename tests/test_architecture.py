...
class TestPipeline(unittest.TestCase):
    ...
    def _packet(self, data: Optional[Dict[str, Any]] = None, source: str = "test"):
    ...
    def test_transform_stage(self):
        def upper_keys(d: Dict[str, Any]):
            return {k.upper(): v for k, v in d.items()}
        p = self.Pipeline("test_t").pipe(self.TransformStage(upper_keys, "upper"))
        result = p.run(self._packet({"hello": "world"}))
        self.assertIn("HELLO", result.data)

    def test_transform_exception_becomes_error(self):
        def boom(d: Dict[str, Any]):
            raise ValueError("kaboom")
        p = self.Pipeline("test_t").pipe(self.TransformStage(boom, "boom"))
    ...
    def test_validate_stage_warns(self):
        def is_inst(v: Any) -> bool:
            return isinstance(v, int)
        rules = {"x": is_inst }
        p = self.Pipeline("test_v").pipe(self.ValidateStage(rules))
...
class TestLayerStack(unittest.TestCase):
    ...
    def setUp(self):
        ...
        class TestLayer(BaseConfigLayer):
            def __init__(self, n: str, p: int, data: Any):
                self.name = n
                self.priority = p
                self._data = data
            def _settings(self):
...
class TestAppearanceLayer(unittest.TestCase):
    ...
    def test_parse_px_helper(self):
        ...
        self.assertEqual(parse_px(" 14 "), 14)
        with self.assertRaises(ValueError):
            parse_px("not-a-number")
